...
def secreto(nombre, defecto=""):
    """Lee un secreto sin que la app se caiga si no hay archivo de secretos.

    `st.secrets.get()` NO se comporta como un `dict.get()`: si no existe ningún
    secrets.toml lanza StreamlitSecretNotFoundError y la app ni siquiera
    arranca — pantalla de error en la primera línea, antes del login. Se vio al
    levantarla en un PC limpio. Acá el valor por defecto es de verdad un valor
    por defecto.
    """
    try:
        v = st.secrets.get(nombre, defecto)
    except Exception:
        return defecto
    return v if v is not None else defecto


# Token de GitHub. Con Supabase activo ya no se usa: queda solo para el camino
# de respaldo, y por eso puede no estar configurado.
GITHUB_TOKEN = secreto("GITHUB_TOKEN", "")
...
SUPABASE_URL      = secreto("SUPABASE_URL", "https://ordgsglujssgzmnlmcus.supabase.co")
SUPABASE_ANON_KEY = secreto(
    "SUPABASE_ANON_KEY",
...
def _render_indicadores(pane=None):
    ...
    _PBI_IND_URL = secreto("PBI_INDICADORES_URL", "").strip()
